...
import com.Ecommerce.Ecommerce.web.application.repository.VendorOrderDetailsRepository;
...
public class VendorOrderDetailsServiceImpl implements VendorOrderDetailsService {
    ...
    public VendorOrderDetails updateVendorOrderDetails(Long id, VendorOrderDetails vendorOrderDetails) {
        ...
        if (vendorOrderDetailsDB != null) {
            if (Objects.nonNull(vendorOrderDetails.getOrderId()) && !"".equalsIgnoreCase(vendorOrderDetails.getOrderId())) {
                vendorOrderDetailsDB.setOrderId(vendorOrderDetails.getOrderId());
            }
            if (Objects.nonNull(vendorOrderDetails.getProductId()) && !"".equalsIgnoreCase(vendorOrderDetails.getProductId())) {
                vendorOrderDetailsDB.setProductId(vendorOrderDetails.getProductId());
            }
            if (Objects.nonNull(vendorOrderDetails.getQuantity()) && !"".equalsIgnoreCase(vendorOrderDetails.getQuantity())) {
